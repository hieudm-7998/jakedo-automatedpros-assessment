...
function SelectLabel(props: React.ComponentProps<typeof SelectPrimitive.Label>) {
  return (
    <SelectPrimitive.Label className="py-1.5 pr-8 pl-2 text-main-foreground/80 text-sm" {...props} />
  )
}
...
function SelectItem({ className, children, ...props }: React.ComponentProps<typeof SelectPrimitive.Item>) {
  return (
    <SelectPrimitive.Item
      className={cn(
        "relative flex items-center gap-2 data-[disabled]:opacity-50 py-1.5 pr-8 pl-2 focus:border-border rounded-base w-full text-sm cursor-default data-[disabled]:pointer-events-none select-none",
        className
      )}
      {...props}
    >
...
function SelectSeparator(props: React.ComponentProps<typeof SelectPrimitive.Separator>) {
  return <SelectPrimitive.Separator className="-mx-1 my-1 bg-border h-px" {...props} />
}
